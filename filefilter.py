...
def printStatus(manager, chunkIndex, totalChunks, rowIndex, rowsInChunk, totalRows, filter_, interactive=False, force=False):
    ...
    message = "Filter " + str(filter_["index"]) + " (" + filter_["name"] + "): " + " Total rows: " + str(
        totalRows) + " Chunk: " + str(chunkIndex) + "/" + str(totalChunks) + " chunk row: " + str(
        rowIndex) + "/" + str(rowsInChunk) + " Queue:" + str(manager.getQueueSize()) + " Workers: " + str(
        manager.getActiveConsumers()) + " avgT:"+ str(statsManager.avg_time(100)) + " ETA: " + str(statsManager.get_eta(rowsInChunk-rowIndex)) + " " + getMemoryUsage()
            ...
            while rows_pending:
                ...
                if exists:
                    ...
                    try:
                        ...
                        try:
                            db.executeQuery("CREATE TABLE filter" + str(filterIndex) + " AS SELECT * FROM read_csv_auto('"+output_file+"')", True)
                        except Exception as e:
                            log.error("Error creating table from file: " + str(e)+ " Exiting because this is an unrecoverable error...")
                            # Exit program
                            exit(1)
